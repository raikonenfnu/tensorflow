...
func @decomposition_in_referenced_functions() {
  ...
  %1 = "tf.Const"() {value = dense<1> : tensor<i32>} : () -> tensor<i32>
  "tf_device.cluster"() ({
    "tf.OpA"(%0, %1) {f = @reachable_function} : (tensor<*x!tf.resource<tensor<2x8xi32>>>, tensor<i32>) -> ()
    tf_device.return
  }) : () -> ()
  return
}
...
func @reachable_function(%arg0: tensor<*x!tf.resource<tensor<2x8xi32>>>, %arg1: tensor<i32>) {
  // CHECK-NOT: "tf.AssignAddVariableOp"
  "tf.AssignAddVariableOp"(%arg0, %arg1) {dtype = "tfdtype$DT_INT32"} : (tensor<*x!tf.resource<tensor<2x8xi32>>>, tensor<i32>) -> ()
  "tf.OpA"(%arg0, %arg1) {f = @reachable_from_reachable_function} : (tensor<*x!tf.resource<tensor<2x8xi32>>>, tensor<i32>) -> ()
  return
}

// CHECK-LABEL: func @reachable_from_reachable_function
func @reachable_from_reachable_function(%arg0: tensor<*x!tf.resource<tensor<2x8xi32>>>, %arg1: tensor<i32>) {
  // CHECK-NOT: "tf.AssignAddVariableOp"
  "tf.AssignAddVariableOp"(%arg0, %arg1) {dtype = "tfdtype$DT_INT32"} : (tensor<*x!tf.resource<tensor<2x8xi32>>>, tensor<i32>) -> ()
  return
}
...
func @unreachable_function(%arg0: tensor<*x!tf.resource<tensor<2x8xi32>>>, %arg1: tensor<i32>) {
  // CLUSTER-ONLY: "tf.AssignAddVariableOp"
  // ALWAYS-DECOMPOSE-NOT:  "tf.AssignAddVariableOp"
  "tf.AssignAddVariableOp"(%arg0, %arg1) {dtype = "tfdtype$DT_INT32"} : (tensor<*x!tf.resource<tensor<2x8xi32>>>, tensor<i32>) -> ()
  return
...
func @decompose_resource_gather_op(%indices : tensor<?xi32>) -> tensor<*xi32> {
  %0 = "tf_device.cluster"() ({
    ...
    // CHECK: return [[GATHER]]
    %1 = "tf.ResourceGather"(%resource, %indices) : (tensor<*x!tf.resource<tensor<*xi32>>>, tensor<?xi32>) -> (tensor<*xi32>)
...
func @decompose_resource_gather_op_subtype(%indices : tensor<5xi32>) -> tensor<2x5x16xi32> {
  %0 = "tf_device.cluster"() ({
    ...
    %1 = "tf.ResourceGather"(%resource, %indices) {batch_dims = 1} : (tensor<*x!tf.resource<tensor<2x8x16xi32>>>, tensor<5xi32>) -> (tensor<2x5x16xi32>)
...
func @decompose_resource_scatter_add_op(%indices : tensor<2x?xi32>, %updates: tensor<?x?x?xi32>) {
  // CHECK: [[CST:%.+]] = "tf.Const"() {value = dense<-1> : tensor<1xi64>} : () -> tensor<1xi64>
  "tf_device.cluster"() ({
    // CHECK: [[VAR:%.+]] = "tf.VarHandleOp"
    %resource = "tf.VarHandleOp"() {container = "c", shared_name = "v"} : () -> tensor<*x!tf.resource<tensor<*xi32>>>

    // CHECK: [[EXPAND:%.+]] = "tf.ExpandDims"([[INDEX]], [[CST]]) : (tensor<2x?xi32>, tensor<1xi64>) -> tensor<2x?x1xi32>
...
func @decompose_resource_scatter_add_op_1d_indices(%indices : tensor<?xi32>, %updates: tensor<?x?x?xi32>) {
  ...
  "tf_device.cluster"() ({
    ...
    // CHECK: [[EXPAND:%.+]] = "tf.ExpandDims"([[INDEX]], [[CST]]) : (tensor<?xi32>, tensor<1xi64>) -> tensor<?x1xi32>
    // CHECK: [[READ:%.+]] = "tf.ReadVariableOp"([[VAR]])
...
func @decompose_variable_shape_i32(%input: tensor<!tf.resource<tensor<?x?x?xf32>>>) -> tensor<3xi32> {
  %0 = "tf_device.cluster"() ({
    %1 = "tf.VariableShape"(%input) : (tensor<!tf.resource<tensor<?x?x?xf32>>>) -> tensor<3xi32>
...
func @decompose_variable_shape_i64(%input: tensor<!tf.resource<tensor<?x?x?xf32>>>) -> tensor<3xi64> {
  %0 = "tf_device.cluster"() ({
    %1 = "tf.VariableShape"(%input) : (tensor<!tf.resource<tensor<?x?x?xf32>>>) -> tensor<3xi64>
    // CHECK: %[[READ:.*]] = "tf.ReadVariableOp"(%arg0)
    // CHECK: %[[SHAPE:.*]] = "tf.Shape"(%[[READ]])
    // CHECK: return %[[SHAPE]]
...
func @decompose_variable_shape_no_subtype(%input: tensor<!tf.resource>) -> tensor<3xi32> {
  %0 = "tf_device.cluster"() ({
    %1 = "tf.VariableShape"(%input) : (tensor<!tf.resource>) -> tensor<3xi32>
...
func @decompose_rng_read_and_skip_op(%resource: tensor<!tf.resource<tensor<3xi64>>>) -> tensor<3xi64> {
  %0 = "tf_device.cluster"() ({
    ...
    %1 = "tf.RngReadAndSkip"(%resource, %alg, %delta) : (tensor<!tf.resource<tensor<3xi64>>>, tensor<i32>, tensor<ui64>) -> tensor<3xi64>
    tf_device.return %1 : tensor<3xi64>
  }) : () -> tensor<3xi64>
  return %0 : tensor<3xi64>
}
